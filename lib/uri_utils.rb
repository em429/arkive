...
def fetch_title(url)
  Net::HTTP.get(URI(url)).scan(%r{<title>(.*?)</title>})[0][0].force_encoding('UTF-8')
  rescue StandardError => e
...
def fetch_readable_content(url, from_archive: false)
  source_uri = Addressable::URI.parse(url)
  if from_archive
    source = URI.open(Addressable::URI.parse("#{IA_GET_API}/#{source_uri}")).read
    base_uri = IA_GET_API
  else
